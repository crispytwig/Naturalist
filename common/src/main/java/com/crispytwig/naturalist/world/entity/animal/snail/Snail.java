package com.crispytwig.naturalist.world.entity.animal.snail;
...
import com.crispytwig.naturalist.world.entity.ai.navigation.CrawlerNavigation;
import com.crispytwig.naturalist.world.entity.SurfaceClimbing;
import com.crispytwig.naturalist.world.entity.SurfaceCrawler;
import com.crispytwig.naturalist.world.entity.ai.control.SurfaceCrawlerMoveControl;
...
public class Snail extends NaturalistAnimal implements Catchable, HidingAnimal, EggLayingAnimal, DataDrivenVariantAnimal, SurfaceCrawler {
    //region Data
    private static final Predicate<ItemStack> FOOD_ITEMS = stack -> stack.is(Items.BEETROOT);
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Animal.createAnimalAttributes().add(Attributes.MAX_HEALTH, 3.0D).add(Attributes.MOVEMENT_SPEED, 0.05F);
    }
    ...
    public void addAdditionalSaveData(@NotNull ValueOutput output) {
        ...
        output.putByte("Color", (byte)this.getSnailColor().getId());
        output.putBoolean("HasEgg", this.hasEgg());
        this.climbing.save(output);
    }
    ...
    public void readAdditionalSaveData(@NotNull ValueInput input) {
        ...
        this.setSnailColor(Color.BY_ID[input.getByteOr("Color", (byte) 0)]);
        this.setHasEgg(input.getBooleanOr("HasEgg", false));
        this.climbing.load(input);
    }
    ...
    public boolean canHide() {
        return NaturalistAnimal.hasVisiblePlayer(this, 5.0D, this.getBoundingBox().inflate(5.0D, 3.0D, 5.0D), EntitySelector.NO_CREATIVE_OR_SPECTATOR::test);
    }
    ...
    public void aiStep() {
        ...
        this.checkCrush();
        if (this.level() instanceof ServerLevel serverLevel && this.isAlive() && !this.isBaby() && --this.slimeBallTime <= 0) {
    ...
    private void checkCrush() {
        if (!(this.level() instanceof ServerLevel serverLevel) || !NaturalistConfig.isSnailCrushingEnabled()
        ...
        if (!players.isEmpty()) {
            this.hurtServer(serverLevel, this.damageSources().playerAttack(players.getFirst()), this.getMaxHealth() * 2.0F);
        }
    }
