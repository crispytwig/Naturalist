...
public class Bass extends AbstractSchoolingFish implements DataDrivenVariantAnimal {
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Animal.createAnimalAttributes().add(Attributes.MAX_HEALTH, 3.0D).add(Attributes.ATTACK_DAMAGE, 1.0D);
    }
    ...
    public static void devour(Mob predator, Bass prey, boolean dropBoneMeal) {
        if (predator.level() instanceof ServerLevel serverLevel) {
            ItemParticleOption bassParticle = new ItemParticleOption(ParticleTypes.ITEM, NaturalistRegistry.BASS.get().asItem());
            for (int i = 0; i < 16; i++) {
